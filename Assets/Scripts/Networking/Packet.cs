...
namespace Networking
{
    ...
    public class Packet : IDisposable
    {
        ...
        {
            buffer.AddRange(BitConverter.GetBytes(_value.Ticks));
        }
        ...
        public DateTime ReadDateTime(bool _moveReadPos = true)
        {
            ...
            {
                // If there are unread bytes
                DateTime _value = DateTime.FromBinary(BitConverter.ToInt64(readableBuffer, readPos)); // Convert the bytes to a bool
                if (_moveReadPos)
                {
                    // If _moveReadPos is true
                    readPos += 1; // Increase readPos by 1
